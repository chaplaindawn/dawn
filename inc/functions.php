 ...
 */
function dawn_get_offset( $limit = '' ) {
 ...
 */
function dawn_get_attachment_meta( $attachment_id = 0 ) {
 ...
 * @return string
 */
function dawn_get_attachment_alt( $attachment_id = 0 ) {

    $r = '';

    if ( $meta = dawn_get_attachment_meta( $attachment_id ) ) :
 ...
 *                      [4] => alt text dawn_get_attachment_alt()
 *
 *         string       path
 */
function dawn_get_image( $args = array() ) {
    ...
    elseif ( $image_data ) :
        ...
        // image alt text
        $r[] = dawn_get_attachment_alt( $image_id );
 ...
 */
function dawn_the_content( $id = '', $format = true, $echo = true ) {
...
if ( ! function_exists( 'dawn_comment' ) ) :
/**
 * Template for comments and pingbacks.
 *
 * Used as a callback by wp_list_comments() for displaying the comments.
 */
function dawn_comment( $comment, $args, $depth ) {
    ...
    <li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
        <div class="comment-body">
            <?php _e( 'Pingback:', 'dawn' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'dawn' ), '<span class="edit-link">', '</span>' ); ?>
        </div>
        ...
        <article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
            <footer class="comment-meta">
                <div class="comment-author vcard">
                    ...
                    <?php printf( __( '%s <span class="says">says:</span>', 'dawn' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
                </div><!-- .comment-author -->

                <div class="comment-metadata">
                    <a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
                        <time datetime="<?php comment_time( 'c' ); ?>">
                            <?php printf( _x( '%1$s at %2$s', '1: date, 2: time', 'dawn' ), get_comment_date(), get_comment_time() ); ?>
                        </time>
                    </a>
                    <?php edit_comment_link( __( 'Edit', 'dawn' ), '<span class="edit-link">', '</span>' ); ?>
                </div><!-- .comment-metadata -->

                <?php if ( '0' == $comment->comment_approved ) : ?>
                <p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'dawn' ); ?></p>
                <?php endif; ?>
            </footer><!-- .comment-meta -->
...
endif; // ends check for dawn_comment()
...
if ( ! function_exists( 'dawn_excerpt' ) ) :
/**
 ...
 * @return str $content
 */
function dawn_excerpt( $limit, $copy = NULL ) {
...
if ( ! function_exists( 'dawn_content' ) ) :
/**
 ...
 * @return str $content
 */
function dawn_content( $limit, $copy = NULL ) {
...
if ( ! function_exists( 'dawn_pagination' ) ) :
/**
...
* @return str $r
*/
function dawn_pagination( $query = false, $echo = true ) {
    ...
    $page_count = dawn_result_count_output( array(
        'query' => $query,
        'echo'  => false
    ) );
...
if ( ! function_exists( 'dawn_result_count' ) ) :
/**
 * Result Count Meta
 * @param  boolean $echo echo or return result
 * @return string        output
 */
function dawn_result_count( $query = false, $echo = true ) {
...
if ( ! function_exists( 'dawn_result_count_output' ) ) :
/**
 * Result Count Meta Output
 * @param array $args options
 * @return string           html output
 */
function dawn_result_count_output( $args = array() ) {
    ...
    $result = dawn_result_count( $query, false );
...
if ( ! function_exists( 'dawn_social_icons' ) ) :
/**
 * Output social icons
 *
 * @param boolean $echo echo or return result
 * @return str $r
 */
function dawn_social_icons( $args = array() ) {
