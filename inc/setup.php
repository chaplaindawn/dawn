...
function dawn_setup() {
     ...
     * If you're building a theme based on Dawn, use a find and replace
     * to change 'dawn' to the name of your theme in all the template files
     */
     ...
     * @return array
     */
    function dawn_media_library_image_options( $sizes ) {
    ...
    add_filter( 'image_size_names_choose', 'dawn_media_library_image_options' );

    /**
     * This theme uses wp_nav_menu() in one location.
     */
    register_nav_menus( array(
        'primary' => __( 'Primary Menu', 'dawn' ),
    ) );
...
function dawn_content_width() {
    $GLOBALS['content_width'] = apply_filters( 'dawn_content_width', 640 );
}
add_action( 'after_setup_theme', 'dawn_content_width', 0 );
 ...
 */
function dawn_add_header_xua() {
    header( 'X-UA-Compatible: IE=edge,chrome=1' );
}
add_action( 'send_headers', 'dawn_add_header_xua' );
...
add_action( 'init', 'dawn_disable_emoji', 1 );
/**
...
function dawn_scripts() {
    ...
    if ( ! is_admin() ) {
        ...
        wp_enqueue_style( 'dawn-style', get_stylesheet_uri(), array( 'main' ) );
 ...
 */
function dawn_remove_recent_comments_style() {
...
add_action( 'widgets_init', 'dawn_remove_recent_comments_style' );
...
add_action( 'admin_init', 'dawn_media_linking', 10 );
 ...
 * Order Menu Items
 */
function dawn_custom_menu_order( $menu_ord ) {
...
add_filter('custom_menu_order', 'dawn_custom_menu_order');
add_filter('menu_order', 'dawn_custom_menu_order');
 ...
 * Modify main query
 */
function dawn_modify_main_query( $query ) {
...
add_action( 'pre_get_posts', 'dawn_modify_main_query', 1 );
...
*/
function dawn__mce_editor_buttons( $buttons ) {

   array_unshift( $buttons, 'styleselect' );
   return $buttons;
}
add_filter( 'mce_buttons_2', 'dawn__mce_editor_buttons' );
...
*/
function dawn__mce_before_init( $settings ) {
...
add_filter( 'tiny_mce_before_init', 'dawn__mce_before_init' );
